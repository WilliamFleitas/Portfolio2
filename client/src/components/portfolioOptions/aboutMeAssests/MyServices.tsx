...
const MyServices = ({ type = 'normalView' }: MyServicesTypeProps) => {
  ...
  return (
    <section
      className={`flex flex-col w-full  text-start  items-center justify-between ${
        type === TYPE_VIEW_OPTIONS.normalView ? 'h-full' : ' h-fit'
      }`}
    >
      <header
        className={`flex flex-col w-full h-fit   ${
          type === TYPE_VIEW_OPTIONS.normalView
            ? 'p-4 border-b border-primaryBorderColor dark:border-secondaryBorderColor'
            : ''
        }`}
      >
      ...
      >
        <div
          className={`${
            type === TYPE_VIEW_OPTIONS.normalView
              ? 'col-span-2 border-b border-primaryBorderColor dark:border-secondaryBorderColor py-4  justify-center'
              : 'keen-slider__slide number-slide1 pt-4 justify-start '
          }  text-start items-start w-full h-full  px-0  flex flex-col`}
        >
          <div
            className={`flex flex-row w-full h-fit text-start items-center justify-start gap-4`}
          >
        ...
        >
          ...
          {type === TYPE_VIEW_OPTIONS.normalView ? (
            <span className='hidden  vertical-gradient-line absolute top-0 left-0 h-[100%] w-[1px] sm:flex flex-col items-center justify-center text-center rounded-[10%]'></span>
          ) : (
            <></>
          )}
        </div>
