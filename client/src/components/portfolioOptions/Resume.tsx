...
export const Experience = ({ type = 'normalView' }: ExperienceTypeProps) => {
    ...
    <div className=' flex flex-col text-start items-center justify-start w-full h-full smd:h-fit'>
      ...
      >
        {jobsData?.map((item, index) => (
          <div
            className={`w-full  text-start items-start justify-start  h-fit py-4   gap-1 ${
              type === TYPE_VIEW_OPTIONS.normalView && index === 2
                ? ' sm:col-span-2'
                : ''
            } ${type === TYPE_VIEW_OPTIONS.normalView ? 'px-4' : 'px-1'}`}
            key={item.id}
          >
            <div className='flex flex-row flex-wrap w-full h-fit sm:h-full text-start items-start justify-between gap-1 pb-2'>
            ...
            >
              <div className=' flex flex-cols flex-row w-full h-fit overflow-hidden gap-2 text-start items-center justify-between no-scrollbar '>
                {item.technologies?.map((item, index) => (
                  <small
                    className={` mx-2 w-fit h-full  whitespace-nowrap  px-2 pt-[3px] m-auto rounded-md bg-primaryMainColor shadow-md font-bold text-zinc-400`}
                    key={index}
                  >
                    {item}
                  </small>
